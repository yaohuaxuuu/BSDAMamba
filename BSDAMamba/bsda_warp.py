...
class BSDAWarp(nn.Module):
    ...
    def __init__(self, backbone, num_classes, info, feature_dim=None, fc_name=None, bsda_kl_weight=8e-4,
                 bsda_recon_weight=1) -> None:
        print("Initializing BSDAWarp...")
        super().__init__()
        ...
        if 'swin' in str(type(self.backbone)).lower():
            print("Detected backbone type: swin")
    ...
    def forward(self, x, is_train=False):
        ...
        # print(a.shape)
        y_hat = self.linear(a)
        # print(f"bsda_war_136_y_hat.shape",y_hat.shape)
        if not is_train:
            return y_hat

        # print("Shape of a before passing to bsda_layer:", a.shape)
        m, mu, logvar, a_hat = self.bsda_layer(a, multi=self.multi)
        # print("BSDA layer output shapes: m:", m.shape, "mu:", mu.shape, "logvar:", logvar.shape, "a_hat:", a_hat.shape)

        a_tilde = self.bsda_layer.calc_a_tilde(a, m, multi=self.multi)
        y_hat_tilde = self.linear(a_tilde)

        # print("Applying BSDAWarp data augmentation")
        # print(y_hat_tilde.shape)

        return (y_hat, y_hat_tilde), (a, a_tilde, a_hat, m, mu, logvar)
        # return y_hat_tilde,(a, a_tilde, a_hat, m, mu, logvar)
